...
namespace NewtonVR
{
    public class NVRHand : MonoBehaviour
    {
        ...
        ObjManagerSeg objManSeg;
        public GameObject lastClosest;
        public float resetTimer;
        public float resetAtTime;
        public bool objPreparedForCut = false;
        public bool cutComplete = false;

        void Start()
        {
            objManSeg = Controller.GetComponent<ObjManagerSeg>();
            resetAtTime = 0.5f;
        }
        ...
        public void HighlightClosestToLeftHand()
        {
            NVRInteractable closest = null;
            float closestDistance = float.MaxValue;

            for (int i = 0; i < objManSeg.Wastes.Count; i++)
            {
                if (objManSeg.Wastes[i] == lastClosest)
                {
                    //Debug.Log("last obj was " + lastClosest.name);
                    lastClosest.GetComponent<Renderer>().material.SetColor("_Color", objManSeg.oldColors[i]);
                    lastClosest = null;
                }
            }

            //if (this.gameObject.name == "LeftHand") Debug.Log("currently hovering over " + CurrentlyHoveringOver.Count + " num of objects");
            ...
            foreach (var hovering in CurrentlyHoveringOver)
            {
                ...
                if (distance < closestDistance)
                {
                    closestDistance = distance;
                    closest = hovering.Key;
                    lastClosest = closest.gameObject;

                    closest.gameObject.GetComponent<Renderer>().material.SetColor("_Color", new Color(0f, 1f, 0f, 0.2f));
                }
            }
        }
        ...
        protected virtual void OnTriggerExit(Collider collider)
        {
            ...
            if (CurrentlyHoveringOver.ContainsKey(interactable) == true)
            {
                if (CurrentlyHoveringOver[interactable].ContainsKey(collider) == true)
                {
                    ...
                    if (objPreparedForCut || cutComplete)
                    {
                        if (cutComplete)
                        {
                            Debug.Log("An object was cut, so resetting CurrentlyHoveringOver");
                            cutComplete = false;
                        }
                         
                        if (objPreparedForCut)
                        {
                            Debug.Log("An object was prepared for cutting, so resetting CurrentlyHoveringOver");
                            objPreparedForCut = false;
                        }
                    ...
                    {
                        //DOESN'T REACH HERE, INTERACTABLE.COUNT DOESN'T REMOVE
